 ...
 * method in the taAllocation.Test class must comply to the following syntax:
 * <pre>
 *     java -classpath sisyphus.jar taAllocation.TAallocation filename maxtime
 * </pre>
 * where:
 * <ul>
 ...
 * 			specificed by the {@link taAllocation.TAallocationPredicates} interface.
...
public class Test {
	...
	static int search(String searchPrg, String datafile, long runtime) {
		...
		if (!prg.exists()) {
			...
			try {
				...
				ZipEntry entry = zip.getEntry("taAllocation/TAallocation.class");
				if (entry==null) {
					println("class taAllocation/TAallocation.class not found in \""+searchPrg+"\".");
		...
		Process proc;
		String  command = "java -classpath "+ searchPrg +" taAllocation.TAallocation "+ datafile +" "+ runtime;
		synchronized (command) {
			try {
				...
				ProcessBuilder pb = new ProcessBuilder("java", "-classpath", searchPrg, 
						"taAllocation.TAallocation", datafile, String.valueOf(runtime));
	...
	protected static int evaluate(String searchPrg, String datafile, long runtime) {
	    ...
	    if (ret >= 0) {
			if (!new File(outfilename).exists()) {
				println("serach program did not generate expected output file \""+outfilename+"\".");
	...
	protected static void evaluate(String searchPrg) {
		TestInst[] tests = {new TestInst("sisyphus.txt"			,10000,	true,  true ),
